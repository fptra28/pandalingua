...
if (isset($_GET['id'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM tbl_course WHERE course_id = $course_id");
...
<body class="m-0 font-sans flex">
    ...
    <div id="content" class="w-full flex flex-col ml-64 p-10 gap-5">
        <a href="../admin-Course.php" class="hover:underline">← BACK</a>
        <div class="flex flex-row justify-between item-center">
            <h1 class="text-2xl font-bold">Add Courses</h1>
        </div>
        <div class="w-full bg-neutral-200 p-5">
            <form action="../controller/editCourse.php" method="post" enctype="multipart/form-data" class="flex flex-col gap-3">
                <input type="hidden" name="course_id" value="<?php echo htmlspecialchars($course['course_id']); ?>">
                <div class="flex flex-col gap-2">
                    <label for="title">Course Title:</label>
                    <input type="text" name="title" id="title" value="<?php echo htmlspecialchars($course['title']); ?>" required class="border-2 border-neutral-500 px-5 py-2 rounded-xl">
                </div>
                <div class="flex flex-col gap-2">
                    <label for="imgCourse">Course Image:</label>
                    <input type="file" name="imgCourse" id="imgCourse" accept="image/*" class="border-2 border-neutral-500 px-5 py-2 rounded-xl">
                </div>
                <div class="flex flex-col gap-2">
                    <label for="total_lessons">Total Lessons:</label>
                    <input type="number" name="total_lessons" id="total_lessons" value="<?php echo htmlspecialchars($course['total_lessons']); ?>" required class="border-2 border-neutral-500 px-5 py-2 rounded-xl">
                </div>
                <button type="submit" class="bg-blue-500 text-white px-5 py-2 rounded-xl hover:bg-blue-900">Update Course</button>
            </form>
        </div>
    </div>
</body>
