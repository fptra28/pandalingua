...
$queryCourses = "SELECT quiz_id, course_id, title, `desc`, created_at FROM tbl_quizzes";
        ...
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($resultCourses)) : ?>
                    <?php
                    // Query untuk menghitung jumlah pertanyaan berdasarkan quiz_id
                    $quizId = $row['quiz_id'];
                    $queryQuestion = "SELECT COUNT(*) AS total FROM questions WHERE quiz_id = '$quizId'";
                    $resultQuestion = mysqli_query($conn, $queryQuestion);
                    $questionData = mysqli_fetch_assoc($resultQuestion);
                    $totalQuestions = $questionData['total'];
                    ?>
                    <tr class="bg-gray-300 text-black text-center">
                        <td class="px-6 py-4"><?php echo htmlspecialchars($row['quiz_id']); ?></td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($row['course_id']); ?></td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($row['title']); ?></td>
                        <td class="px-6 py-4 w-1/2"><?php echo htmlspecialchars($row['desc']); ?></td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($totalQuestions); ?> Questions</td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($row['created_at']); ?></td>
                        <td class="px-6 py-4 flex flex-col gap-2 h-full">
                            <a href="./admin-question.php?quiz_id=<?php echo htmlspecialchars($row['quiz_id']); ?>" class="w-full bg-yellow-500 text-black py-2 px-10 hover:bg-yellow-900 hover:text-white">Question</a>
                            <a href="./quiz/editQuiz.php?id=<?php echo htmlspecialchars($row['quiz_id']); ?>" class="w-full bg-blue-500 text-white py-2 px-10 hover:bg-blue-900">Edit</a>
                            <a href="./controller/deleteQuiz.php?id=<?php echo htmlspecialchars($row['quiz_id']); ?>" class="w-full bg-red-500 text-white py-2 px-10 hover:bg-red-900">Delete</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
